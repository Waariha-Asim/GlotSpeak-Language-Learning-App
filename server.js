...
app.post('/api/conversation/claude-chat', authMiddleware, async (req, res) => {
  try {
    ...
    const prompt = `You are an AI language tutor in: "${scenarioContext}". 
Rules:
- Act as a native English speaker in this scenario
...
Your response:`;
    ...
    // Use Gemini 2.5 Flash model - try these exact names
    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash", // Primary try
      // model: "gemini-2.0-flash", // Alternative
      // model: "gemini-2.0-flash-lite", // Another option
      generationConfig: {
        maxOutputTokens: 150,
        temperature: 0.7,
      }
    ...
    console.log('✅ Gemini 2.5 Flash Response:', aiResponse);

    history.push({ role: 'user', text: message });
    history.push({ role: 'ai', text: aiResponse });
    if (history.length > 10) history.splice(0, history.length - 10);

    res.json({ success: true, message: aiResponse, conversationLength: history.length });
  } catch (error) {
    console.error('❌ Gemini 2.5 Flash API error:', error);

    // Try alternative model names automatically
    try {
      ...
      res.json({ success: true, message: altResponse, conversationLength: history.length });

    } catch (altError) {
      console.error('❌ All models failed:', altError);
      ...
      const fallbackResponses = [
        ...
        "That's a good point. How can I help you practice further?"
      ];
...
app.post('/api/conversation/feedback', authMiddleware, async (req, res) => {
  try {
    ...
    if (history.length === 0) {
      return res.json({
        success: true,
        feedback: "No conversation history found. Please have a conversation first to get feedback."
      });
    }

    const feedbackPrompt = `Analyze this English conversation and provide constructive feedback...`;
